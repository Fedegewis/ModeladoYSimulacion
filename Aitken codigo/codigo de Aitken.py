...
def newton_raphson(f: Callable[[float], float], x0: float, df: Optional[Callable[[float], float]] = None,
    ...
    for n in range(max_iter):
        ...
        x1 = x - fx / dfx
        # calcular x2 (la siguiente iteración desde x1)
        dfx1 = df(x1) if df else numerical_derivative(f, x1)
        x2 = x1 - f(x1) / dfx1 if abs(dfx1) > 1e-14 else None

        abs_err = abs(x1 - x)
        rel_err = abs_err / abs(x1) if x1 != 0 else float('inf')
        history.append((n, x, fx, dfx, abs_err, rel_err, x1, x2))
...
def punto_fijo(g: Callable[[float], float], x0: float, tol: float = 1e-8, max_iter: int = 50):
    ...
    for n in range(max_iter):
        ...
        abs_err = abs(x1 - x)
        rel_err = abs_err / abs(x1) if x1 != 0 else float('inf')
        history.append((n, x, x1, abs_err, rel_err, x1, x2))
        if abs_err < tol:
            return x1, history
        x = x1
    return None, history
...
def punto_fijo_aitken(g: Callable[[float], float], x0: float, tol: float = 1e-8, max_iter: int = 50):
    ...
    for n in range(max_iter):
        ...
        # guardamos todo: n, x, x1, x2, x_acc
...
class RaicesGUI:
    ...
    def _build_widgets(self):
        ...
        ttk.Button(frm, text="Limpiar", command=self.clear_all).grid(row=4, column=3)

        # Tabla
        self.tree = ttk.Treeview(frm, columns=(), show='headings', height=10)
    ...
    def _set_table_columns(self, cols):
    ...
    def run_newton(self):
        ...
        self._set_table_columns(("n", "x_n", "f(x)", "f'(x)", "err_abs", "err_rel", "x_{n+1}", "x_{n+2}"))
    ...
    def run_punto_fijo(self):
        ...
        self._set_table_columns(("n", "x_n", "aprox", "err_abs", "err_rel", "x_{n+1}", "x_{n+2}"))
        self._populate_table(hist)
        self.result_var.set(f"Resultado: {root}" if root else "No convergió")
    ...
    def run_aitken(self):
        ...
        self._set_table_columns(("n", "x_n", "x_{n+1}", "x_{n+2}", "x_Aitken", "err_abs", "err_rel"))
        self._populate_table(hist)
