...
def resolver_sistema(num):
    ...
    if len(eigenvalues) >= 2:
        ...
        if lam1.has(sp.I):
            ...
            print("\nSOLUCIÓN HOMOGÉNEA:")
            print(f"X_h(t) = e^({alpha}t)·[ c1·({v_real}·cos({beta}t) - {v_imag}·sin({beta}t))")
            print(f"                       + c2·({v_real}·sin({beta}t) + {v_imag}·cos({beta}t)) ]")
        else:
            X_h = c1 * sp.exp(lam1 * t) * v1 + c2 * sp.exp(lam2 * t) * v2
            
            print("\nSOLUCIÓN HOMOGÉNEA:")
            # Formatear para que se vea como vectores columna
            v1_str = sp.pretty(v1)
            v2_str = sp.pretty(v2)
            print(f"X_h(t) = c1·e^({lam1}t)·{v1_str} + c2·e^({lam2}t)·{v2_str}")
